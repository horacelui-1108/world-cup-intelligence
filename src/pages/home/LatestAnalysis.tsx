...
import { ANALYSIS_SOURCES, COMPACT_ANALYSES, FEATURED_ANALYSIS } from './data';
import { useTimezone } from '@/lib/timezone';
import { kickoffLabel } from '@/lib/format';

/** Home Section 6 — 最新賽後分析 */
export default function LatestAnalysis() {
  const { timeZone } = useTimezone();
  const featured = FEATURED_ANALYSIS;

  return (
    <section aria-labelledby="analysis-heading" className="mx-auto max-w-7xl px-4 py-6 md:px-6 md:py-10">
      <SectionHeader id="analysis-heading" title="最新賽後分析" linkLabel="全部分析" linkTo="/analysis" />
            ...
            <div className="p-5">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-caption text-text-3">
                ...
                <span className="inline-flex items-center gap-0.5">
                  {ANALYSIS_SOURCES.slice(0, 2).map((s, i) => (
                    <SourceTag key={s.name} index={i + 1} source={s} />
                  ))}
                  <span className="text-text-3">等 {featured.sourceCount} 個來源</span>
                </span>
              </div>
            </div>
          </Link>
