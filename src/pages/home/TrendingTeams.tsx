...
import { TRENDING_TEAMS } from './data';
import { Crest } from '@/components/TeamChip';
import { useFavorites } from '@/lib/favorites';
import { cn } from '@/lib/utils';

const formColor: Record<'W' | 'D' | 'L', string> = { W: 'bg-win', D: 'bg-draw', L: 'bg-live' };
const formLabel: Record<'W' | 'D' | 'L', string> = { W: '勝', D: '和', L: '負' };
...
export default function TrendingTeams() {
  ...
  return (
    <section aria-labelledby="teams-heading" className="mx-auto max-w-7xl px-4 py-6 md:px-6 md:py-10">
      <SectionHeader id="teams-heading" title="熱門球隊" caption="依關注度與近期表現排序" />
      <motion.ul
      ...
      >
        {TRENDING_TEAMS.map(({ team, form, keyStat }) => {
          ...
          return (
            ...
            >
              ...
              <Link to={`/teams/${team.id}`} className="flex flex-col items-center px-4 pt-6 pb-5 text-center">
                ...
                <span className="mt-0.5 text-caption text-text-3">FIFA 排名第 {team.ranking}</span>
                <span
